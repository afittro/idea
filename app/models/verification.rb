...
class Verification < ApplicationRecord
  ...
  belongs_to :employee
  belongs_to :revision
  belongs_to :a1_doc, class_name: "DocumentType", foreign_key: "a1_doc_id"
  belongs_to :a2_doc, class_name: "DocumentType", foreign_key: "a2_doc_id"
  belongs_to :a3_doc, class_name: "DocumentType", foreign_key: "a3_doc_id"
  ...
  def str
    "#{self.last_name}, #{self.first_name} #{self.middle_name}|SSN #{self.ssn}|DoB #{self.born_on}".strip
  end
  ...
  def revision_string
    self.revision.starts_on.to_s
  end

  def document_type_name(doc)
    doc.abbr
  end
